...
屏幕端对齐 `src/ws63_laser_host_ui/` 的主控语义，但不复制 PC 工具职责：

| 对齐项 | 屏幕端做法 |
|--------|------------|
| 状态词 | 使用“空闲 / 数据传输中 / 任务就绪 / 正在执行 / 已终止 / 错误”等 Host 同源语义 |
| 控制语义 | 保持“执行 / 暂停 / 放弃 / FOCUS_OFF”与 Host Job 页一致 |
| RX 真相源 | 页面只展示模型状态，不用本地按钮伪造激光状态 |
| 信息架构 | Home、任务控制、系统设置、诊断日志、SD 任务与 Host 主流程对应 |
| 非目标 | 不在屏幕端加入 PC 串口配置、G-code 编辑、AI 生图或日志文件管理 |
