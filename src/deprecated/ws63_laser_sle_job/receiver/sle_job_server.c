...
static sle_job_packet_rx_cb_t g_packet_cb = NULL;
static sle_job_disconnect_cb_t g_disconnect_cb = NULL;
...
static errcode_t sle_job_server_add(void)
{
...
static void sle_enable_cbk(errcode_t status)
{
    ...
    errcode_t ret = sle_job_server_add();
    if (ret != ERRCODE_SLE_SUCCESS) {
        return;
    }
