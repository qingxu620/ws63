...
u16 Lcd_ReadData_16Bit(void)
{
	...
	LCD_CS_CLR;
	LCD_RS_CLR;	  
	SPI_WriteByte(SPI2,lcddev.rramcmd);
	SPI_SetSpeed(SPI2,0);
	LCD_RS_SET;
...
u16 LCD_Read_ID(void)
{
	...
	{
		LCD_RS_CLR;	  
		SPI_WriteByte(SPI2,0xFB);
		LCD_RS_SET;
		SPI_WriteByte(SPI2,0x10+i);
		LCD_RS_CLR;	  
		SPI_WriteByte(SPI2,0xD3);
		SPI_SetSpeed(SPI2,0);
		LCD_RS_SET;
		val[i-1] = SPI_WriteByte(SPI2,0xFF);
		SPI_SetSpeed(SPI2,1);
		LCD_RS_CLR;	  
		SPI_WriteByte(SPI2,0xFB);
		LCD_RS_SET;
		SPI_WriteByte(SPI2,0x00);	
	}
	LCD_CS_SET;
	LCD_WR_REG(0xF0);     // Command Set Control
	LCD_WR_DATA(0x3C);   
