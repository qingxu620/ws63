...
static int start_softap(void)
{
    ...
    td_char ifname[WIFI_IFNAME_MAX_SIZE + 1] = LASER_WIFI_AP_IFNAME;
    ...
    (void)memcpy_s(hapd_conf.pre_shared_key, WIFI_MAX_KEY_LEN, LASER_WIFI_AP_PSK,
                   strlen(LASER_WIFI_AP_PSK) + 1);
    hapd_conf.security_type = 3;
    hapd_conf.channel_num = LASER_WIFI_AP_CHANNEL;
    ...
    IP4_ADDR(&ipaddr, LASER_WIFI_AP_IP_A, LASER_WIFI_AP_IP_B, LASER_WIFI_AP_IP_C, LASER_WIFI_AP_IP_D);
    IP4_ADDR(&netmask, 255, 255, 255, 0);
    IP4_ADDR(&gw, LASER_WIFI_AP_IP_A, LASER_WIFI_AP_IP_B, LASER_WIFI_AP_IP_C, LASER_WIFI_AP_GW_D);
...
static int start_tcp_server(void)
{
    ...
    int opt = 1;
    int sock_buf = LASER_WIFI_TCP_SOCK_BUF_SIZE;
    ...
    server_addr.sin_port = htons(LASER_WIFI_TCP_PORT);
    ...
    if (listen(g_listen_sock, LASER_WIFI_TCP_BACKLOG) < 0) {
...
int task_wifi_grbl_entry(void *arg)
{
    ...
    while (1) {
        ...
        uint8_t buf[LASER_WIFI_TCP_RX_BUF_SIZE];
