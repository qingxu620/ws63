...
2. 接收侧新增命令参数校验（坐标范围、速度有限值、非法命令拒收）。
3. `ack_seq` 语义与注释已统一为“已接收(入队)最新序号”。
4. `Kconfig` 与 `common/config.h` 已打通（关键总线/通道/CS 可由 `CONFIG_*` 覆盖）。
5. 发射板首条业务命令不再使用 `seq=0`，避免初始 `ack=0` 误判。
6. 发射板心跳会给业务命令让路，降低 SSAP 在途窗口争抢。
7. 接收板 `Run/Idle` 判定已修正，不再把“激光开但不动”长期报成 `Run`。
8. 接收板安全监控已改为连续超时确认，避免单次抖动直接停光。
