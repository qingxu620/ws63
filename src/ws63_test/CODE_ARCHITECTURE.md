...
4. `zdt_controller`：张大头 `Emm_V5.0` 的底层 UART/协议驱动与 bring-up 代码目录，供 `focus_node` 复用，不单独作为板卡角色。
5. `focus_node`：感知与对焦节点第一版，也是当前唯一的 `Z` 轴节点角色，把 `TTL UART + ZDT Z轴` 整成一个独立节点，后续再叠 `NFC / 测高 / 自动对焦`。
