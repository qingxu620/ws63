...
static void send_grbl_startup(const char *source)
{
    ...
    uart_send_str("\r\nWS63 Wireless Laser RX Bring-up V1.0\r\n");
...
static bool handle_dollar_command(const char *line)
{
    ...
    if (strncmp(line, "$J=", 3) == 0) {
        ...
        uart_send_str("[VER:1.1f.WS63_WIRELESS_RX_BRINGUP:]\r\n[OPT:V,15,128]\r\nok\r\n");
        ...
        uint16_t qfree = (depth >= MOTION_QUEUE_SIZE) ? 0U : (uint16_t)(MOTION_QUEUE_SIZE - depth);
        ...
        uart_send_str(buf);
        snprintf(buf, sizeof(buf),
                 "[MSG:wireless rx conn=%u hb=%lu cmd=%lu ack=%u qfree=%u]\r\nok\r\n",
                 (unsigned int)(sle_laser_server_get_conn_id() != 0xFFFFU),
                 (unsigned long)sle_laser_server_get_heartbeat_rx_count(),
                 (unsigned long)sle_laser_server_get_business_rx_count(),
                 (unsigned int)sle_laser_server_get_last_ack_seq(),
                 (unsigned int)qfree);
        uart_send_str(buf);
