/**
 * @file main.c
 * @brief Integrated RX route-based R3B entry.
 */
#include "app_init.h"
#include "dac8562.h"
...
static void laser_rx_unified_entry(void)
{
    ...
    osal_printk("  Route-based integration R3B\r\n");
    osal_printk("========================================\r\n");
    ...
    osal_printk("[RX_INTEGRATED] legacy_uart compiled but not started in R3B\r\n");
#endif
#if defined(CONFIG_LASER_RX_TRANSPORT_WIFI)
    osal_printk("[RX_INTEGRATED] R3B start legacy_wifi route\r\n");
    if (!route_manager_set_active(RX_ROUTE_LEGACY_WIFI)) {
        osal_printk("[RX_INTEGRATED] start legacy_wifi failed\r\n");
        laser_force_off();
        return;
    }
...
#endif
    osal_printk("[RX_INTEGRATED] SLE route not started\r\n");
}
