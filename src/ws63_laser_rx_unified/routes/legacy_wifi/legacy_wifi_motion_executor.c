...
{
    errcode_t ret = dac8563_write_xy(mm_to_dac(g_current_x, LEGACY_WIFI_BEILV_X),
                                    mm_to_dac(g_current_y, LEGACY_WIFI_BEILV_Y));
    if (ret != ERRCODE_SUCC) {
        osal_printk("[LEGACY_WIFI_MOTION] DAC write failed ret=0x%x\r\n", ret);
        laser_force_off();
