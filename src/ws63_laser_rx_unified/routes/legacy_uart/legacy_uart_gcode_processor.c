/**
 * @file legacy_uart_gcode_processor.c
...
static double g_feed_rate = LEGACY_UART_DEFAULT_FEED_RATE;
...
static void fill_cmd_header(legacy_uart_motion_cmd_t *cmd, uint8_t type)
{
    memset(cmd, 0, sizeof(legacy_uart_motion_cmd_t));
    cmd->cmd = type;
    cmd->seq = g_seq++;
    if (g_laser_enabled) {
    ...
    if (g_absolute_mode) {
        cmd->flags |= LEGACY_UART_FLAG_ABS_MODE;
    }
}

static bool append_laser_off(legacy_uart_motion_cmd_t *out_cmds, int max_cmds, int *count)
{
    if (out_cmds == NULL || count == NULL || *count >= max_cmds) {
        return false;
    }

    fill_cmd_header(&out_cmds[*count], LEGACY_UART_CMD_LASER_OFF);
...
{
    g_feed_rate = LEGACY_UART_DEFAULT_FEED_RATE;
    g_laser_power = 0.0;
...
void legacy_uart_gcode_processor_build_emergency_stop(legacy_uart_motion_cmd_t *out_cmd)
{
    ...
    fill_cmd_header(out_cmd, LEGACY_UART_CMD_EMERGENCY_STOP);
    out_cmd->laser_pwr = 0;
    g_laser_enabled = false;
    g_laser_power = 0.0;
}

static bool is_word_start(const legacy_uart_gcode_line_t *gc, int pos, char letter)
{
...
bool legacy_uart_gcode_process_line(const char *line, int len, legacy_uart_motion_cmd_t *out_cmds, int max_cmds, int *out_count)
{
    ...
    *out_count = 0;
    ...
    for (int i = 0; i < len && i < LEGACY_UART_GCODE_LINE_MAX - 1; i++) {
    ...
    if (legacy_uart_gcode_has_word(&gc, 'F')) {
        double f = legacy_uart_gcode_get_value(&gc, 'F');
        if (f > 0.0) {
            g_feed_rate = f;
            line_processed = true;
        }
    ...
    if (legacy_uart_gcode_has_word(&gc, 'S')) {
        double s = legacy_uart_gcode_get_value(&gc, 'S');
        if (s < 0.0) {
            s = 0.0;
        }
        if (s > LEGACY_UART_LASER_S_MAX) {
            s = LEGACY_UART_LASER_S_MAX;
        }
        g_laser_power = s;
        line_processed = true;
        if (s <= 0.0) {
    ...
    if (legacy_uart_gcode_has_word(&gc, 'M')) {
        int m_val = (int)legacy_uart_gcode_get_value(&gc, 'M');
        if (m_val == 3 || m_val == 4) {
    ...
    for (int i = 0; i < gc.len; i++) {
        ...
        if (g_val == 90) {
            ...
            fill_cmd_header(&out_cmds[count], LEGACY_UART_CMD_SET_ORIGIN);
            count++;
            legacy_uart_gcode_processor_set_origin();
    ...
    if (!suppress_modal_motion && (legacy_uart_gcode_has_word(&gc, 'X') || legacy_uart_gcode_has_word(&gc, 'Y')) && count < max_cmds) {
        ...
        fill_cmd_header(&out_cmds[count], (g_motion_mode == 0) ? LEGACY_UART_CMD_G0_MOVE : LEGACY_UART_CMD_G1_MOVE);
        out_cmds[count].target_x = (float)target_x;
        out_cmds[count].target_y = (float)target_y;
        out_cmds[count].feed_rate = (g_motion_mode == 0) ? (float)LEGACY_UART_G0_FEED_RATE : (float)g_feed_rate;
