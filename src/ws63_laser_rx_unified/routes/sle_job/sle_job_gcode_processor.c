/**
 * @file sle_job_gcode_processor.c
...
static double g_feed_rate = SLE_JOB_DEFAULT_FEED_RATE;
...
static void fill_cmd_header(sle_job_motion_cmd_t *cmd, uint8_t type)
{
    memset(cmd, 0, sizeof(sle_job_motion_cmd_t));
    cmd->cmd = type;
    cmd->seq = g_seq++;
    if (g_laser_enabled) {
        cmd->flags |= SLE_JOB_FLAG_LASER_ON;
    }
    if (g_absolute_mode) {
        cmd->flags |= SLE_JOB_FLAG_ABS_MODE;
    }
}

static bool append_laser_off(sle_job_motion_cmd_t *out_cmds, int max_cmds, int *count)
{
    if (out_cmds == NULL || count == NULL || *count >= max_cmds) {
        return false;
    }

    fill_cmd_header(&out_cmds[*count], SLE_JOB_CMD_LASER_OFF);
...
{
    g_feed_rate = SLE_JOB_DEFAULT_FEED_RATE;
    g_laser_power = 0.0;
    ...
    g_seq = 1;
    g_line_counter = 0;
    sle_job_gcode_processor_set_origin();
}
...
void sle_job_gcode_processor_build_emergency_stop(sle_job_motion_cmd_t *out_cmd)
{
    ...
    fill_cmd_header(out_cmd, SLE_JOB_CMD_EMERGENCY_STOP);
    out_cmd->laser_pwr = 0;
    g_laser_enabled = false;
    g_laser_power = 0.0;
}

static bool is_word_start(const sle_job_gcode_line_t *gc, int pos, char letter)
{
...
bool sle_job_gcode_process_line(const char *line, int len, sle_job_motion_cmd_t *out_cmds, int max_cmds, int *out_count)
{
    ...
    *out_count = 0;
    ...
    sle_job_gcode_init(&gc);
    for (int i = 0; i < len && i < SLE_JOB_GCODE_LINE_MAX - 1; i++) {
    ...
    if (sle_job_gcode_has_word(&gc, 'F')) {
        double f = sle_job_gcode_get_value(&gc, 'F');
        if (f > 0.0) {
            g_feed_rate = f;
            line_processed = true;
        }
    ...
    if (sle_job_gcode_has_word(&gc, 'S')) {
        double s = sle_job_gcode_get_value(&gc, 'S');
        if (s < 0.0) {
            s = 0.0;
        }
        if (s > SLE_JOB_LASER_S_MAX) {
            s = SLE_JOB_LASER_S_MAX;
        }
        g_laser_power = s;
        line_processed = true;
        if (s <= 0.0) {
    ...
    if (sle_job_gcode_has_word(&gc, 'M')) {
        int m_val = (int)sle_job_gcode_get_value(&gc, 'M');
        if (m_val == 3 || m_val == 4) {
    ...
    if (!suppress_modal_motion && (sle_job_gcode_has_word(&gc, 'X') || sle_job_gcode_has_word(&gc, 'Y')) && count < max_cmds) {
        double target_x = g_plan_x;
        double target_y = g_plan_y;

        if (sle_job_gcode_has_word(&gc, 'X')) {
            double x = sle_job_gcode_get_value(&gc, 'X');
            target_x = g_absolute_mode ? x : (g_plan_x + x);
        }
        if (sle_job_gcode_has_word(&gc, 'Y')) {
            double y = sle_job_gcode_get_value(&gc, 'Y');
            target_y = g_absolute_mode ? y : (g_plan_y + y);
        }

        fill_cmd_header(&out_cmds[count], (g_motion_mode == 0) ? SLE_JOB_CMD_G0_MOVE : SLE_JOB_CMD_G1_MOVE);
        out_cmds[count].target_x = (float)target_x;
        out_cmds[count].target_y = (float)target_y;
        out_cmds[count].feed_rate = (g_motion_mode == 0) ? (float)SLE_JOB_G0_FEED_RATE : (float)g_feed_rate;
