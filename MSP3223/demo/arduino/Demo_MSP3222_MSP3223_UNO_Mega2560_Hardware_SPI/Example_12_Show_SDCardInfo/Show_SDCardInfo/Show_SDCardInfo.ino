...
//  Serial.print("\nInitializing SD card...");
  ...
  show_string("Card type:",10,10,1,WHITE, BLACK,0);
  switch(card.type()) {
    case SD_CARD_TYPE_SD1:
      show_string("SD1",70,10,1,WHITE, BLACK,0);
      break;
    case SD_CARD_TYPE_SD2:
      show_string("SD2",70,10,1,WHITE, BLACK,0);
      break;
    case SD_CARD_TYPE_SDHC:
      show_string("SDHC",70,10,1,WHITE, BLACK,0);
      break;
    default:
      show_string("Unknown",70,10,1,WHITE, BLACK,0);
  }
  ...
  uint32_t volumesize;
  show_string("Volume type is FAT",10,30,1,WHITE, BLACK,0);
  mylcd.Print_Number_Int(volume.fatType(), 120, 30, 0, ' ', 10);
 
  volumesize = volume.blocksPerCluster();    // clusters are collections of blocks
  volumesize *= volume.clusterCount();       // we'll have a lot of clusters
  volumesize *= 512;                            // SD card blocks are always 512 bytes
  
  show_string("size (Kbytes): ",10,70,1,WHITE, BLACK,0);
  volumesize /= 1024;
  mylcd.Print_Number_Int(volumesize, 95, 70, 0, ' ', 10);

  show_string("size (Mbytes): ",10,90,1,WHITE, BLACK,0);
  volumesize /= 1024;
  mylcd.Print_Number_Int(volumesize, 95, 90, 0, ' ', 10);
}
